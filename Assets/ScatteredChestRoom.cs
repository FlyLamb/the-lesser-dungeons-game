...
public class ScatteredChestRoom : Room {
    ...
    public Entity[] scatteredObjects;
    ...
    public override void Setup() {
        ...
        for(int i = 0; i < Random.Range(0,14); i++) {
    ...
    public override GameObject GenCell(int x, int y, Vector3 ePos, float seed) {
        ...
        if (prl > 0.62f) {
            if (prl > 0.7)
                ...
                return AddEntity(scatteredObjects[Random.Range(0, scatteredObjects.Length)], ePos, Quaternion.identity);
        } else {
