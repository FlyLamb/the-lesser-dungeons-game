...
public class Room : MonoBehaviour {
    ...
    public void GenerateDoors() {
        if (n) {
            doorN = Instantiate(doorPrefab, transform.position + new Vector3(0, 0, 4), Quaternion.Euler(0, 0, 0),transform).GetComponent<Door>();
            doorN.leadsTo = myPosition + new Vector2Int(0, 1);
            doorN.roomIn = this;
        }
        if (s) {
            doorS = Instantiate(doorPrefab, transform.position + new Vector3(0, 0, -4), Quaternion.Euler(0, 180, 0), transform).GetComponent<Door>();
            doorS.leadsTo = myPosition + new Vector2Int(0, -1);
            doorS.roomIn = this;
        }
        if (e) {
            doorE = Instantiate(doorPrefab, transform.position + new Vector3(6, 0, 0), Quaternion.Euler(0, 90, 0), transform).GetComponent<Door>();
            doorE.leadsTo = myPosition + new Vector2Int(1, 0);
            doorE.roomIn = this;
        }
        if (w) {
            doorW = Instantiate(doorPrefab, transform.position + new Vector3(-6, 0, 0), Quaternion.Euler(0, 270, 0), transform).GetComponent<Door>();
    ...
    public GameObject AddEntity(Entity e, Vector3 position, Quaternion rotation) {
        GameObject g = Instantiate(e.gameObject, position, rotation, transform);
